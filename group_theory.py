...
def product_operation(op1, op2):
    return lambda p: (op1(p[0]), op2(p[1]))
...
def test_isomorphism(generators_1, generators_2, op_1, op_2):
    ...
    proj_x = set()
    proj_x = set()
    product_subgroup = generate_finite_semigroup(generators, product_operation(op_1, op_2), lambda candidate_tuple: line_test_helper(candidate_tuple, proj_x, proj_y))
...
def line_test_helper(candidate_tuple, proj_x, proj_y):
    if (candidate_tuple[0] in proj_x) or (candidate_tuple[1] in proj_y):
        return False

    proj_x.add(candidate_tuple[0])
    proj_y.add(candidate_tuple[1])
